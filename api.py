...
def process_data(i, data, llm, client, llm_to_api, response_dict, args):
    system_prompt = """You are a sophisticated AI. You are provided with the following:

1. A graph Problem.
2. The optimal answer to the problem.
3. The requirement when you solve this problem.

Your task is to solve the problem step by step and output the answer in the required format. Besides, your answer should be started with 'Let's think step by step:'. Also note that your task is to find the optimal ansqwer, so you should output the optimal answer, not to verify the optimal answer."""
# 2. A hint to solve this problem.
    ...
    try:
        ...
        response_dict[str(i)][llm]['input'] = data['input']
        response_dict[str(i)][llm]['output'] = chat_completion.choices[0].message.content
...
def main(args, datas, llm, client, llm_to_api, response_dict):
    with concurrent.futures.ThreadPoolExecutor(max_workers=args.num_workers) as executor:
        futures = []
        for i in range(args.st, args.ed):
            data = datas[i]
            response_dict[str(i)] = {}
            futures.append(executor.submit(process_data, i, data, llm, client, llm_to_api, response_dict, args))
...
if __name__ == '__main__':
    ...
    for llm in args.llm.split('-'):
        ...
        args.ed = len(datas)
        #args.ed = 29
        main(args, datas, llm, client, llm_to_api, response_dict)
